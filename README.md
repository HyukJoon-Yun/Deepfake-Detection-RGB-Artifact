...
전체 데이터셋과 최종 학습 모델 체크포인트는 용량 문제로 GitHub 저장소에 직접 포함하지 않았다.
...
다운로드한 `data` 폴더와 `models` 폴더는 프로젝트 폴더 안에 그대로 배치하여 사용할 수 있도록 하였다.
...
- 제한적인 데이터셋 규모
- 특정 딥페이크 생성 방식에 대한 데이터 편향 가능성
- Artifact Map에 조명, 화장, 피부 질감, 압축 노이즈 등이 함께 반영될 가능성
- 정지 이미지 기반 실험으로 인한 영상의 temporal artifact 분석 한계

### 7.2 향후 개선 방향

- 다양한 딥페이크 생성 방식의 데이터 추가
- 외부 데이터셋을 활용한 일반화 성능 검증
- 얼굴 crop 및 alignment 적용을 통한 얼굴 영역 중심 탐지 성능 개선
- 주파수 영역 분석 및 temporal artifact 분석 추가
...
- RGB와 Artifact Map의 상보적 특징 학습 확인
- RGB + Artifact 결합 입력에서 단일 입력 대비 성능 향상
- 초기 모델 비교 실험 기준 EfficientNet-B4 앙상블 정확도 93.5% 기록
- 데이터셋 정제 후 최종 재평가 기준 EfficientNet-B4 앙상블 정확도 96.00% 기록
- Artifact Map을 통한 RGB 이미지 내 미세한 조작 흔적 보완 효과 확인
